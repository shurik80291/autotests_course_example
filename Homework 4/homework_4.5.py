...
def josephus_task(num_people, kill_num):
    people = [j for j in range(1, num_people + 1)]
    kill_index = 0
    while len(people) > 1:
        kill_index += kill_num - 1
        if kill_index > len(people) - 1:
            while kill_index > len(people) - 1:
                kill_index -= len(people)
        del people[kill_index]
    survivor = people[0]
    return survivor
