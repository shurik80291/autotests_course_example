...
def everything_for_your_cat(cats_data):
    """
    Оптимизация хранения данных о кошках и их владельцах
    :param cats_data: список кортежей (книга о котах и покупателях), где каждый кортеж состоит из:
    Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя
    :return: строка, где для каждого владельца на одной строке отображаются данные о всех его животных,
    на новой строке следующий покупатель и все его питомцы и т.д.
    """
    our_str = ''
    for index_book in range(len(cats_data)):
        if cats_data[index_book][2] and cats_data[index_book][3] in our_str:
            continue
        our_str += f'{cats_data[index_book][2]} {cats_data[index_book][3]}: ' \
                   f'{cats_data[index_book][0]}, {cats_data[index_book][1]}'
        for index_next in range(index_book + 1, len(cats_data)):
            if cats_data[index_next][2:] == cats_data[index_book][2:]:
                our_str += f'; {cats_data[index_next][0]}, {cats_data[index_next][1]}'
        our_str += '\n'
    return our_str
