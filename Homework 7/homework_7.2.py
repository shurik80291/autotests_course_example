...
class PersonInfo:
    ...
    def path_deps(self):
        """
        Находим путь до подразделения
        :return: строка "Головное подразделение --> ... --> Конечное подразделение"
        """
        return ' --> '.join(self.subdivision)

    def new_salary(self):
        """
        Индексация зарплаты. Высчитывается по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        (регистр имеет значение "А" и "а" - разные буквы)
        ...
        letters_list = []
        while subdivision != '':
            letters_list.append(subdivision.count(subdivision[0]))
            subdivision = subdivision.replace(subdivision[0], '')
        letters_list.sort(reverse=True)
        return 1337 * self.age * sum(letters_list[:3])
