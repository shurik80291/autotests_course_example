...
class RomanNums:
    def __init__(self, roman):
        """
        Атрибуты класса RomanNums
        :param roman: строка - римское число
        """
        self.roman = roman
    ...
    def from_roman(self):
        ...
        arabic = 0
        for index, number in enumerate(self.roman):
            if len(self.roman[index:]) == 1:
                arabic += roman_dict.get(number)
            elif roman_dict.get(number) >= roman_dict.get(self.roman[index + 1]):
                arabic += roman_dict.get(number)
            else:
                arabic -= roman_dict.get(number)
        return arabic
    ...
    def is_palindrome(self):
        """
        Проверка, является ли арабское число палиндромом,
        т.е. имеет ли одинаковое значение число при чтении слева направо и справа налево
